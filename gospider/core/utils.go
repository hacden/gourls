...
func GetExtType(rawUrl string) string {
	...
	jsfiles := []string{"index.","config.","app.","data.","user.","sswd.","admin.","file.","pro.","login.","setting"}

	if fileExt == ".js" || fileExt == ".xml" || fileExt == ".json" || fileExt == ".map"{
		for _,i := range jsfiles{
			if strings.Contains(u.Path,i){
				return fileExt
			}
...
func GetExttoSave(rawUrl string) string {
	extfiles := []string{".css",".png",".gov.cn"}

	for _,i := range extfiles{
		if strings.Contains(rawUrl,i){
			return ""
		}
