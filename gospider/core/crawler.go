...
import (
	...
	"gourls/gospider/stringset"
	"github.com/spf13/cobra"

)

var Gospiderlist []string
...
func NewCrawler(site *url.URL) *Crawler {
...
func (crawler *Crawler) feedLinkfinder(jsFileUrl string, OutputType string, source string, ) {
	...
	if !crawler.jsSet.Duplicate(jsFileUrl) {
		...
		if GetExtType(jsFileUrl)!=""{
			Gospiderlist = append(Gospiderlist, jsFileUrl)
		}
...
func (crawler *Crawler) Start(linkfinder bool) {
	...
	crawler.C.OnHTML("[href]", func(e *colly.HTMLElement) {
		...
		if !crawler.urlSet.Duplicate(urlString) {
			...
			if GetExtType(urlString)!=""{
				Gospiderlist = append(Gospiderlist, urlString)
			}
			if crawler.JsonOutput {
			sout := SpiderOutput{
				Input:      crawler.Input,
	...
	crawler.C.OnHTML("form[action]", func(e *colly.HTMLElement) {
		...
		if !crawler.formSet.Duplicate(formUrl) {
			...
			if GetExtType(formUrl)!=""{
				Gospiderlist = append(Gospiderlist, formUrl)
			}
			if crawler.JsonOutput {
				sout := SpiderOutput{
					Input:      crawler.Input,
	...
	crawler.C.OnHTML(`input[type="file"]`, func(e *colly.HTMLElement) {
		...
		if !uploadFormSet.Duplicate(uploadUrl) {
			...
			if GetExtType(uploadUrl)!=""{
				Gospiderlist = append(Gospiderlist, uploadUrl)
			}
			if crawler.JsonOutput {
				sout := SpiderOutput{
					Input:      crawler.Input,
	...
	crawler.C.OnResponse(func(response *colly.Response) {
		...
		if len(crawler.filterLength_slice) == 0 || !contains(crawler.filterLength_slice,len(respStr)) {
			...
			if GetExtType(u)!=""{
				Gospiderlist = append(Gospiderlist, u)
			}
			if crawler.length {
			...
			if crawler.raw { 
				...
				if GetExtType(respStr)!=""{
					Gospiderlist = append(Gospiderlist, respStr)
				}
				if !crawler.Quiet { 
					fmt.Println(outputFormat)
				}
...
func (crawler *Crawler) findSubdomains(resp string) {
	...
	for _, sub := range subs {
		if !crawler.subSet.Duplicate(sub) {
			...
			if GetExtType(sub)!=""{
				Gospiderlist = append(Gospiderlist, sub)
			}
			if crawler.JsonOutput {
				sout := SpiderOutput{
					Input:      crawler.Input,
...
func (crawler *Crawler) findAWSS3(resp string) {
	...
	for _, e := range aws {
		if !crawler.awsSet.Duplicate(e) {
			...
			if GetExtType(e)!=""{
				Gospiderlist = append(Gospiderlist, e)
			}
			if crawler.JsonOutput {
				sout := SpiderOutput{
					Input:      crawler.Input,
			...
			if InScope(response.Request.URL, crawler.C.URLFilters) {			
				...
				for _, relPath := range paths {
					...
					if crawler.JsonOutput {
						...
						if GetExtType(relPath)!=""{
							Gospiderlist = append(Gospiderlist, relPath)
						}
					}
					fmt.Println(outputFormat)
					...
					if fileExt == ".js" || fileExt == ".xml" || fileExt == ".json" || fileExt == ".map" {
						...
						if crawler.JsonOutput {
							...
							if GetExtType(rebuildURL)!=""{
								Gospiderlist = append(Gospiderlist, rebuildURL)
							}
						}
						...
						if fileExt == ".js" || fileExt == ".xml" || fileExt == ".json" || fileExt == ".map" {
							...
							if crawler.urlSet.Duplicate(urlWithJSHostIn){
								...
								if crawler.JsonOutput {
									...
									if GetExtType(urlWithJSHostIn)!=""{
										Gospiderlist = append(Gospiderlist, urlWithJSHostIn)
									}
